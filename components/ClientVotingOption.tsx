...
export function ClientVotingOption(props: ClientVotingOptionProps) {
    ...
    return (<div className="flex flex-col md:flex-row justify-between items-center gap-8">
        <VotingStatistics
            votes={{ posVotes, neutralVotes, negVotes }}
        />

        <div className="h-16 w-48 md:h-48 md:w-16" id={`parent-${props.movieEventId}`}>
            <VotingWidget givenVote={givenVote} movieEventId={props.movieEventId} sendVote={sendVote} />
        </div>
    </div>
    )
}