...
export async function fetchMoviesDetailFromExternalAPI(movieAPIIds: number[]): Promise<Movie[]> {
    ...
    const promises = movieAPIIds.map(id => fetch(
        `https://api.themoviedb.org/3/movie/${movieAPIIds[0]}`,
        {headers: {
