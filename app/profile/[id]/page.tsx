...
export default async function ProfilePage() {
  ...
  const { _avg, _min, _max, _count } = aggregateStats;
  ...
  return (
    <div className="flex flex-col gap-5 px-5 items-stretch">
      ...
      <div className="px-5 py-2  rounded-md text-lg">
        ...
        <ul className="mt-4">
          ...
          <li>
            Highest rating: {_max.score} (for{' '}
            <Link href={`/movies/${minMovie?.id}`}>
              {minMovie?.movie.title}
            </Link>
            )
          </li>
          <li>
            Lowest rating: {_min.score} (for{' '}
            <Link href={`/movies/${maxMovie?.id}`}>
              {maxMovie?.movie.title}
            </Link>
            )
          </li>
        </ul>
...
function ReviewComponent({ movie, review }: { movie: Movie; review: Review }) {
  const textReviewEl = (
    <div className="flex flex-col gap-1 mt-2">
      ...
      <p className="py-2 px-4 rounded prose dark:prose-invert prose-sm bg-bg-100/20 dark:bg-dark-bg-100/20">
        {review.reviewText}
      </p>
    </div>
  );
  ...
  return (
    <section className="text-base py-2 px-5 flex flex-col gap-1  rounded-md h-full bg-primary-100 dark:bg-dark-primary-100">
      ...
      {review.reviewText != null &&
        ...
        textReviewEl}
      <Link
        className="btn border-4 border-accent-100 dark:border-dark-accent-100 hover:bg-accent-100 hover:dark:bg-dark-accent-100"
        href={`/movies/${movie.id}/review`}
      >
        Update review
      </Link>
    </section>
