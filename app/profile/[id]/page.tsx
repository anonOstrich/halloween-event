...
export default async function ProfilePage() {
    ...
    const numericalScores = reviews.map(r => convertScoreToNumber(r.score))
    ...
    const maxReview = reviews.find(r => convertScoreToNumber(r.score) == max)!
    const maxMovie = maxReview.movie.title
    const minReview = reviews.find(r => convertScoreToNumber(r.score) == min)!
...
function Review({ movie, review }: { movie: Movie, review: Review }) {
    return <section className="border border-red-600 text-base p-2 flex flex-col gap-1">
        ...
        <span>Score: {convertScoreToNumber(review.score)} / 19</span><br />
        <div>
            <span>Review:</span>
