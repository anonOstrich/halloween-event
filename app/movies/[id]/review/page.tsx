...
async function handleReviewSubmission(data: FormData) {
  ...
  const updatedReview = await reviewMovie(
    movieId,
    reviewScore,
    reviewText ?? null
  );
...
export default async function ReviewPage({
  ...
  const defaultScore =
    possibleExistingReview == null ? 10 : possibleExistingReview.score;

  // TODO: this is not the most elegant syntax
  const defaultText =
    possibleExistingReview == null
      ? ''
      : possibleExistingReview.reviewText ?? '';

  return (
    <section>
      <h1 className="text-center prose dark:prose-invert prose-2xl">
        {movie.title}
      </h1>
      <form action={handleReviewSubmission} className="form">
        ...
        <FormRow
          ...
          name="movie-review-text"
        />
        <ExperimentalThumbsUpWidget
          config={{ min: 0, max: 19, label: 'Score' }}
          input={{ score: defaultScore }}
        />
        <div></div>

        <div className="w-full flex justify-center items-center gap-5">
          <button type="submit" className="">
            {possibleExistingReview == null ? 'Review' : 'Update review'}
          </button>
          {possibleExistingReview == null ? (
            <Link className="block" href={`/movies/${movie.id}`}>
              Cancel
            </Link>
          ) : (
            <form action={handleReviewDeletion}>
