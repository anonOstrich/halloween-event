...
export default async function MoviePage({
  ...
  return (
    <main className="flex flex-col justify-between max-w-2xl mx-auto gap-10">
...
function DeleteMovieComponent({ movieId }: DeleteMovieComponentProps) {
  return (
    <div className="mx-5 my-5 bg-danger dark:bg-dark-danger rounded-full min-w-[50px] min-h-[50px] flex justify-center align-middle shadow-lg">
      <form
...
function UpdateMovieComponent({ movie }: UpdateMovieComponentProps) {
  ...
  return (
    <Link
      href={`/movies/${id}/edit`}
      className=" block border-2 bg-accent-100 dark:bg-dark-accent-100 py-1 px-2 rounded-md shadow-md"
    >
      Update
    </Link>
  );
